...
<div class="content-wrapper">
        ...
        <div class="card">
            ...
            <div class="card-header">
                ...
                <div class="card-tools">
                    ...
                    <button class="btn btn-primary btn-sm" type="button" data-toggle="modal" data-target="#modalAddDocumentation-{{$document[0]['idCategorie']}}">
                        <i class="fa fa-plus mr-2" aria-hidden="true"></i> Ajouter {{$document[0]['categorie']}}
                    </button>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td width="30%" class="text-center">
                                ...
                                <a class="btn btn-info btn-sm" data-toggle="modal" data-target="#modalUpdate-{{ $doc['id'] }}">
                                    <i class="fas fa-pencil-alt">
                                    </i>
                                </a>
                                <a class="btn btn-danger btn-sm" data-toggle="modal" data-target="#modalDelete-{{ $doc['id'] }}">
    ...
    @foreach($document as $doc)
    <!-- ------------------------------------------------------------------------------------------- Modal Ajout Traités-->
    <div class="modal fade" id="modalAddDocumentation-{{$document[0]['idCategorie']}}" tabindex="-1" role="dialog" aria-hidden="true">
            ...
            <div class="modal-content">
                <form action="/documentation" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="modal-header">
                        <h3 class="modal-title" style="text-transform: uppercase;"> AJOUT D'UNE DOCUMENTATION {{$document[0]['categorie']}}</h3>
                        ...
                        <div class="container">
                            <div class="shadow p-4">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="hidden" name="idCategorie" value="{{$document[0]['idCategorie']}}">
                                    <span id="categorie" class="form-control" style="background-color: #e9ecef;">{{$document[0]['categorie']}}</span>

                                </div>

                                <div class="form-group">
                                    <label>Type Catégorie <sup class="text-danger">*</sup> </label>
                                    <select class="form-control" name="idTypeCategorie" required>
                                        <option value="">Choix Type Categorie</option>
                                        @foreach($typeCategories as $typeCategorie)
                                        @if($typeCategorie['id'])
                                        @if($document[0]['idCategorie'] == $typeCategorie['idCategorie'])
                                        <option value="{{$typeCategorie['id']}}">{{$typeCategorie['titre']}}</option>
                                        @endif
                                        @endif
                                        @endforeach
                                    </select>
    ...
    @foreach($documents as $idCategorie => $document)
    @foreach($document as $doc)
    <div class="modal fade" id="modalUpdate-{{ $doc['id'] }}" tabindex="-1" role="dialog" aria-hidden="true">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <form action="/documentation" method="POST" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="modal-header">
                        <h3 class="modal-title" style="text-transform: uppercase;"> MODIFICATION DE LA DOCUMENTATION {{$document[0]['categorie']}}</h3>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                        ...
                        <div class="container">
                            <div class="shadow p-4">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" name="titre" id="titre" value="{{ $doc['titre'] }}" required>
                                </div>

                                <div class="form-group">
                                    <label>Catégorie <sup class="text-danger">*</sup> </label>
                                    <input type="hidden" name="idCategorie" value="{{$doc['idCategorie']}}">
                                    <span id="categorie" class="form-control" style="background-color: #e9ecef;">{{$doc['categorie']}}</span>
                                </div>

                                <div class="form-group">
                                    <label>Type Catégorie <sup class="text-danger">*</sup> </label>
                                    <select class="form-control" name="idTypeCategorie" required>
                                        <option value="{{ $doc['idTypeCategorie'] }}">{{ $doc['typeCategorie'] }}</option>
                                        @foreach($typeCategories as $typeCategorie)
                                        @if($typeCategorie['id'])
                                        @if($doc['idCategorie'] == $typeCategorie['idCategorie'])
                                        <option value="{{$typeCategorie['id']}}">{{$typeCategorie['titre']}}</option>
                                        @endif
                                        @endif
                                        @endforeach
                                ...
                                <div class="form-group">
                                    <label for="date">Date de publication <sup class="text-danger">*</sup> </span></label> <span class="text-secondary"> <i> {{ $doc['date'] }} </i>
                                        <input type="date" class="form-control" name="date" id="date" value="{{ $doc['date'] }}">
                                </div>
    ...
    @foreach($documents as $idCategorie => $document)
    @foreach($document as $doc)
    <div class="modal fade" id="modalDelete-{{$doc['id']}}" tabindex="-1" role="dialog" aria-hidden="true">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <form action="/archive_documentation" method="POST" enctype="multipart/form-data">
                    ...
                    @method('DELETE')
                    <div class="modal-header">
                        <h5 class="modal-title" style="text-transform: uppercase;">SUPPRIMER LA DOCUMENTATION {{$doc['categorie']}}</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                        ...
                        <div class="container">
                            <div class="shadow p-4">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="hidden" name="id" value="{{$doc['id']}}">
                                    <input type="text" class="form-control text-danger" name="titre" id="titre" value="{{$doc['categorie']}}: {{$doc['titre']}}" readonly>
                                    <input type="hidden" name="idCategorie" value="{{$doc['idCategorie']}}">
                                    <input type="hidden" name="idTypeCategorie" value="{{$doc['idTypeCategorie']}}">
                                    <input type="hidden" name="date" value="{{$doc['date']}}">
                                    <input type="hidden" name="fichier" value="{{$doc['fichier']}}">
